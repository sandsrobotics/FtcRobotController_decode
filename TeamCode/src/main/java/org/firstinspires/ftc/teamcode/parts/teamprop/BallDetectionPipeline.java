...
public class BallDetectionPipeline extends OpenCvPipeline
{
    ...
    static final Point REGION2_TOPLEFT_ANCHOR_POINT = new Point(500,350);
    ...
    public Mat processFrame(Mat input)
    {
         ...
         */
        //edit; nah ignore what we JUST said above, really now we are seeing the H value and determining if it's green.
        // just so you know, for future years if you want to see the colors, just use this:
        //Core.mean(*insert your mat or submat here*).val[0]. This takes the mean of each of the individual
        //HSV values of your target submat (or mat!) and then filters it out for only the H value. (H = Hue)
        //variable time!
        ...
        //Now we are checking if the main color of the submat is green.
            if (Core.mean(region1).val[0] > 34 && Core.mean(region1).val[0] < 91) // Was green from region 1?
            {
            ...
            } if (Core.mean(region2).val[0] > 34 && Core.mean(region2).val[0] < 91) // Was it from region 2?
            {
                ...
                Imgproc.rectangle(
                        ...
                        GREEN, // The color the rectangle is drawn in
                        5); // Negative thickness means solid fill
            } if (Core.mean(region3).val[0] > 34 && Core.mean(region3).val[0] < 91) // Was it from region 3?
            {
            ...
            } if (Core.mean(region1).val[0] > 120 && Core.mean(region1).val[0] < 165) {

                //make a variable; is the ball purple?
                boolean isPurple1 = true;
                ...
                Imgproc.rectangle(
                        ...
                        5); // Negative thickness means solid fill
            } if (Core.mean(region2).val[0] > 120 && Core.mean(region2).val[0] < 165) {

                //make a variable; is the ball purple?
                boolean isPurple2 = true;
                ...
                Imgproc.rectangle(
                        ...
                        PURPLE, // The color the rectangle is drawn in
                        5); // Negative thickness means solid fill
            } if (Core.mean(region3).val[0] > 120 && Core.mean(region3).val[0] < 165) {

                //make a variable; is the ball purple?
                boolean isPurple3 = true;
            ...
            } if (ispurple1 = false && position != TeamPropPosition.LEFT) {
                Imgproc.rectangle(
                        input, // Buffer to draw on
                        region1_pointA, // First point which defines the rectangle
                        region1_pointB, // Second point which defines the rectangle
                        ...
                        5); // Negative thickness means solid fill
            } if (ispurple2 = false && position != TeamPropPosition.CENTER) {
                Imgproc.rectangle(
                        input, // Buffer to draw on
                        region2_pointA, // First point which defines the rectangle
                        region2_pointB, // Second point which defines the rectangle
                        WHITE, // The color the rectangle is drawn in
                        5); // Negative thickness means solid fill
            } if (ispurple3 = false && position != TeamPropPosition.RIGHT) {
                Imgproc.rectangle(
                    input, // Buffer to draw on
                    region3_pointA, // First point which defines the rectangle
                    region3_pointB, // Second point which defines the rectangle
