...
@Config
@Autonomous (name="14273.4 LKTest Auto000", group="C14273")
public class T1_LKTest_Auto000 extends LinearOpMode{
...
//    Vector3 p_fieldStart                 = new Vector3(63,16,180); // x:64
    Vector3 p_fieldStart                 = new Vector3(0,0,180); // x:64
    ...
    public void runOpMode() {
        ...
        DecodeSettings.lkTestMode1 = false; //true;
        DecodeSettings.lkTestMode2 = false;
        DecodeSettings.lkNoCam = true;
        DecodeSettings.lkNoLL = true;
...
//        testNewAuto(autoTasks);
        ...
        while (opModeIsActive()) {
            ...
            StateMachine.addTelemetry(telemetry);
            telemetry.update();
        }
        robot.stop();
    }

    protected void testLK(TimedTask autoTasks) {
        // reminder: X is toward audience, Y is away from red team
        Vector3 l_pos1          = new Vector3(-12, 0,180);
        Vector3 l_pos2          = new Vector3(-24, 16,180);
        Vector3 l_pos3         = new Vector3(-42, 6, -135);
        Vector3 l_pos4        = new Vector3(-24, 16, -90);
        Vector3 l_pos5          = new Vector3(-12,0,0);
        Vector3 l_pos6          = new Vector3(0,0,180);
        ...
        positionSolver.addMoveToTaskEx(l_pos1, autoTasks);
        positionSolver.addMoveToTaskEx(l_pos2, autoTasks);
        autoTasks.addStep(() -> positionSolver.setSettings(PositionSolverSettings.defaultSettings));
        positionSolver.addMoveToTaskEx(l_pos3, autoTasks);
        autoTasks.addDelay(2000);
        autoTasks.addStep(() -> positionSolver.setSettings(PositionSolverSettings.loseSettings));
        positionSolver.addMoveToTaskEx(l_pos4, autoTasks);
        positionSolver.addMoveToTaskEx(l_pos5, autoTasks);
        autoTasks.addStep(() -> positionSolver.setSettings(PositionSolverSettings.defaultSettings));
        positionSolver.addMoveToTaskEx(l_pos6, autoTasks);
        autoTasks.addDelay(4000);
        autoTasks.addStep(() -> positionSolver.stopSolver());

        autoTasks.addStep(() -> parts.autoDrive.addNavTargets(toTargetTransition(l_pos1)) );
        autoTasks.addStep(() -> parts.autoDrive.addNavTargets(toTargetTransition(l_pos2)) );
        autoTasks.addStep(() -> parts.autoDrive.addNavTargets(toTargetAccurate(l_pos3)) );
        autoTasks.addStep(() -> !parts.autoDrive.isNavigating);
        autoTasks.addDelay(2000);
        autoTasks.addStep(() -> parts.autoDrive.addNavTargets(toTargetTransition(l_pos4)) );
        autoTasks.addStep(() -> parts.autoDrive.addNavTargets(toTargetTransition(l_pos5)) );
        autoTasks.addStep(() -> parts.autoDrive.addNavTargets(toTargetAccurate(l_pos6)) );
        autoTasks.addStep(() -> !parts.autoDrive.isNavigating);
        autoTasks.addDelay(4000);
        autoTasks.addStep(() -> parts.autoDrive.stop());
    }

    protected void testLK2(TimedTask autoTasks) {
        // reminder: X is toward audience, Y is away from red team
        Vector3 l_pos1          = new Vector3(-12, 8,150);
        Vector3 l_pos2          = new Vector3(-24, 16,180);
        Vector3 l_pos3          = new Vector3(-36, 8,-135);
        Vector3 l_pos4          = new Vector3(-48, 0,-90); //here
        Vector3 l_pos5          = new Vector3(-48, 16,180);
        Vector3 l_pos6         = new Vector3(-36, 16, 180);
        Vector3 l_pos7        = new Vector3(-24, 8, 150);
        Vector3 l_pos8          = new Vector3(-12,0,180);
        Vector3 l_pos9          = new Vector3(0,0,180);
        ...
        positionSolver.addMoveToTaskEx(l_pos5, autoTasks);
        positionSolver.addMoveToTaskEx(l_pos6, autoTasks);
        positionSolver.addMoveToTaskEx(l_pos7, autoTasks);
        positionSolver.addMoveToTaskEx(l_pos8, autoTasks);
        autoTasks.addStep(() -> positionSolver.setSettings(PositionSolverSettings.defaultSettings));
        positionSolver.addMoveToTaskEx(l_pos9, autoTasks);
        autoTasks.addDelay(2000);
        autoTasks.addStep(() -> positionSolver.stopSolver());

        autoTasks.addStep(() -> parts.autoDrive.addNavTargets(toTargetTransition(l_pos1)) );
        autoTasks.addStep(() -> parts.autoDrive.addNavTargets(toTargetTransition(l_pos2)) );
        autoTasks.addStep(() -> parts.autoDrive.addNavTargets(toTargetTransition(l_pos3)) );
        autoTasks.addStep(() -> parts.autoDrive.addNavTargets(toTargetAccurate(l_pos4)) );
        autoTasks.addStep(() -> !parts.autoDrive.isNavigating);
        autoTasks.addDelay(2000);
        autoTasks.addStep(() -> parts.autoDrive.addNavTargets(toTargetTransition(l_pos5)) );
        autoTasks.addStep(() -> parts.autoDrive.addNavTargets(toTargetTransition(l_pos6)) );
        autoTasks.addStep(() -> parts.autoDrive.addNavTargets(toTargetTransition(l_pos7)) );
        autoTasks.addStep(() -> parts.autoDrive.addNavTargets(toTargetTransition(l_pos8)) );
        autoTasks.addStep(() -> parts.autoDrive.addNavTargets(toTargetAccurate(l_pos9)) );
        autoTasks.addStep(() -> !parts.autoDrive.isNavigating);
        autoTasks.addDelay(2000);
        autoTasks.addStep(() -> parts.autoDrive.stop());
...
//    }
    ...
    public void extraSettings() {
        ...
        if (this.getClass().isAnnotationPresent(TeleOp.class)) {
            opModeName = this.getClass().getAnnotation(TeleOp.class).name();
        } else if (this.getClass().isAnnotationPresent(Autonomous.class)) {
            opModeName = this.getClass().getAnnotation(Autonomous.class).name();
        }
...
//        DecodeSettings.setCurrentOpMode("T1_AutoFarRed");
        DecodeSettings.setCurrentOpMode(opModeName);
